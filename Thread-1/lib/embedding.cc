...
EmbeddingHolder* EmbeddingHolder::deep_copy(){
    EmbeddingHolder* eh_copy = new EmbeddingHolder();
    for (unsigned int i = 0; i < get_n_embeddings(); ++i) {
        proj1::Embedding* temp = new proj1::Embedding(get_embedding(i)); 
        eh_copy->append(temp);
    }
    return eh_copy;
}
